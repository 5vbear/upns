...
public class UpnsAgentManagerImpl extends AbstractUpnsManager implements UpnsAgentManager, Schema {
    ...
    @Override
    public Configuration getConfiguration(String input) {
        String userId = input;
        ...
        if (StringUtils.isNotBlank(userId)) {
            ...
            configuration.setRegisterUrl(registerUrl);
    ...
    public Routing getRouting(final String userId) {
        return getHttpTemplate().execute(new UpnsHttpCallback<Routing>() {
            ...
            protected HttpRequestBase getHttpMethod() {
                return new HttpGet(getConfiguration(userId).getRegisterUrl());
            }
        });
    }
    ...
    private int getResourceId(String name, String type) {
        int id = context.getResources().getIdentifier(name,
                type, applicationInfo.packageName);
        if (id == 0) {
            throw new IllegalArgumentException("can not find resource of name:" + name + ",type:" + type + "," +
                    "make sure you have copy necessary resource of upns ");
        }
//        Logger.debug("found resource with name:" + name + ",type:" + type + ",resourceId is:" + id);
        return id;
    }
    ...
    private void createNotification(String userId, String applicationId) {
        ...
        Notification notification = new Notification(getResourceId("message_notification", "drawable"),
                "您有新的未读消息:" +
                        item.getContent(), System.currentTimeMillis());
        notification.flags |= Notification.FLAG_AUTO_CANCEL;
        notification.defaults |= Notification.DEFAULT_ALL;

        RemoteViews remoteView = new RemoteViews(applicationInfo.packageName, getResourceId("notification", "layout"));
        remoteView.setImageViewBitmap(getResourceId("applicationView", "id"),
                BitmapUtils.fromByteArray(application.getNotificationIcon()));
        remoteView.setTextViewText(getResourceId("titleView", "id"), item.getTitle());
        remoteView.setTextViewText(getResourceId("contentView", "id"), item.getContent());
        remoteView.setTextViewText(getResourceId("receiveTimeView", "id"), TIME_FORMAT.format(
...
//    }
    ...
    public MessageRetrieveResult retrieveMessages(String userId, String applicationId, long timestamp, int size) {
        Logger.info("尝试获取历史消息,userId:" + userId + ",applicationId:" + applicationId + ",timestamp:" + timestamp
                + ",size:" + size);
